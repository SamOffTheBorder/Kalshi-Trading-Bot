...
- [x] 8.2 Record results in the change; propose calibrated go/no-go thresholds — runs #3-#8 (2026-07-15/16) recorded in notes.md. Iteration found and fixed four structural defects: entry clustering (→ `EntryThrottle`), perma-stuck PAUSE (→ trailing-peak guard), zero-drift model fading trends (→ `trend_zscore` gate), and unbounded fills (→ 25%-of-bar-volume liquidity cap). Run #8 (honest fills): first positive OOS margin (+10.2%, 64 trades, Sharpe 0.72) — still below the Sharpe>1.0 bar, and the test window was peeked during iteration
- [x] 8.3 Docs: architecture + runbook pages in docs-site for fetching data and running a backtest
- [ ] 8.4 **Frozen-config validation (gate decider)**: config frozen as of run #8 (trend z 1.5/24h, throttle 3/24h, liquidity cap 25%). Re-run ~2026-07-28+ on archived days that did not exist at freeze time (and KXETH/KXETHD once covered) with NO tuning. Margin holds → finalize go/no-go thresholds and archive this change; margin dies → park crypto_mispricing, pivot to weather/econ strategy proposal
